...
export default function Collection({ 
  ...
  const sortAndFilter = (list: any[]) => {
    let filtered = list;
    if (filterRace !== 'ALL') {
      filtered = filtered.filter(h => h.race === filterRace);
    }
    if (filterLevel !== 'ALL') {
      filtered = filtered.filter(h => h.level >= (filterLevel as number));
    }
  ...
  const sortedHeroes = sortAndFilter(player.collection);
  const sortedOffsprings = sortAndFilter(player.offsprings);
  ...
  return (
    ...
    >
      ...
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-2 bg-zinc-900/50 p-2 rounded-xl border border-white/5">
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
          ...
          <select 
            value={filterRace} 
            onChange={(e) => setFilterRace(e.target.value as any)}
            className="bg-transparent text-xs font-bold outline-none cursor-pointer"
          >
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
          ...
          >
            ...
            <option value="20">20级以上</option>
            <option value="50">50级以上</option>
            <option value="100">100级以上</option>
          </select>
        </div>
        <button 
          onClick={() => setShowBulkExecute(true)}
          className="ml-auto px-4 py-1.5 bg-red-500/20 hover:bg-red-500/30 text-red-500 border border-red-500/30 rounded-lg text-xs font-bold transition-all flex items-center gap-2"
        >
      ...
      <div className="flex-1 overflow-y-auto pr-2">
        {tab === 'heroes' ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
            {player.petCollection.map((pet) => (
              <PetCard 
                ...
                onRename={(name) => onRenameHero(pet.id, name)} // Reuse rename for pets
              />
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {sortedOffsprings.map((offspring) => (
              <OffspringCard 
                key={offspring.id}
                offspring={offspring}
...
function PetCard({ pet, isActive, onClick, onRename }: { pet: Pet, isActive: boolean, onClick: () => void, onRename: (name: string) => void }) {
  ...
  return (
    <div className={cn(
      ...
      <div className="text-4xl mb-3 cursor-pointer" onClick={onClick}>{pet.icon}</div>
      {isEditing ? (
        <div className="flex items-center gap-1">
          <input 
            ...
            className="bg-black/40 border border-white/20 rounded px-1 py-0.5 text-[10px] font-bold w-full outline-none"
            onKeyDown={e => e.key === 'Enter' && (onRename(tempName), setIsEditing(false))}
          />
        </div>
      ) : (
        <div className="flex items-center gap-1 group/name">
          <div className="text-sm font-bold truncate w-full cursor-pointer" onClick={onClick}>{pet.name}</div>
          <button onClick={() => setIsEditing(true)} className="opacity-0 group-hover/name:opacity-100 p-1 hover:bg-white/10 rounded">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-1">
              ...
              <button 
                ...
                className={cn("p-1 rounded transition-colors", hero.isLocked ? "text-red-400 bg-red-400/10" : "text-white/20 hover:bg-white/10")}
              >
                {hero.isLocked ? <Lock className="w-3 h-3" /> : <Unlock className="w-3 h-3" />}
              </button>
              <button 
              ...
              <button 
                ...
                className={cn(
                  ...
                  isActive ? "text-emerald-500 bg-emerald-500/10" : "text-white/20 hover:bg-white/10"
                )}
              >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-1">
              ...
              <button 
                onClick={(e) => { e.stopPropagation(); onToggleLock(); }}
                className={cn("p-1 rounded transition-colors", offspring.isLocked ? "text-red-400 bg-red-400/10" : "text-white/20 hover:bg-white/10")}
              >
                {offspring.isLocked ? <Lock className="w-3 h-3" /> : <Unlock className="w-3 h-3" />}
              </button>
            </div>
          </div>
          ...
          <div className="text-[10px] text-white/40 mt-1">性别: {offspring.gender}</div>
