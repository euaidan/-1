...
interface ShopScreenProps {
  ...
  onExchange: (mode: 'gem_to_gold' | 'gold_to_gem', amount: number) => void;
}

export default function ShopScreen({ player, onBuyItem, onExchange }: ShopScreenProps) {
  const [gemToGoldAmount, setGemToGoldAmount] = React.useState(1);
  const [goldToGemAmount, setGoldToGemAmount] = React.useState(1);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {shopItems.map(item => {
          const ownedCount = player.inventory.find(i => i.type === item.type)?.count || 0;
          return (
            <div key={item.type} className="bg-zinc-900/50 border border-white/10 rounded-3xl p-6 flex flex-col items-center text-center group hover:bg-zinc-800 transition-all relative">
              {ownedCount > 0 && (
                <div className="absolute top-4 right-4 bg-emerald-500 text-black text-[10px] font-bold px-2 py-0.5 rounded-full">
                  持有: {ownedCount}
                </div>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-black/20 p-6 rounded-2xl border border-white/5 flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold">{gemToGoldAmount} 钻石</span>
              </div>
              <span className="text-white/20">→</span>
              <div className="flex items-center gap-2">
                <Coins className="w-5 h-5 text-yellow-400" />
                <span className="font-bold">{gemToGoldAmount * 100} 金币</span>
              </div>
            </div>
            <input 
              type="range" 
              min="1" 
              max={Math.max(1, player.gems)} 
              value={gemToGoldAmount} 
              onChange={(e) => setGemToGoldAmount(parseInt(e.target.value))}
              className="w-full accent-emerald-500"
            />
            <button
              onClick={() => {
                onExchange('gem_to_gold', gemToGoldAmount);
                setGemToGoldAmount(1);
              }}
              disabled={player.gems < gemToGoldAmount}
          ...
          <div className="bg-black/20 p-6 rounded-2xl border border-white/5 flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <Coins className="w-5 h-5 text-yellow-400" />
                <span className="font-bold">{goldToGemAmount * 150} 金币</span>
              </div>
              <span className="text-white/20">→</span>
              <div className="flex items-center gap-2">
                <Gem className="w-5 h-5 text-emerald-400" />
                <span className="font-bold">{goldToGemAmount} 钻石</span>
              </div>
            </div>
            <input 
              type="range" 
              min="1" 
              max={Math.max(1, Math.floor(player.gold / 150))} 
              value={goldToGemAmount} 
              onChange={(e) => setGoldToGemAmount(parseInt(e.target.value))}
              className="w-full accent-emerald-500"
            />
            <button
              onClick={() => {
                onExchange('gold_to_gem', goldToGemAmount);
                setGoldToGemAmount(1);
              }}
              disabled={player.gold < goldToGemAmount * 150}
              className="w-full py-3 rounded-xl border border-white/10 hover:bg-white/5 font-bold transition-all disabled:opacity-50"
            >
              立即兑换
            </button>
